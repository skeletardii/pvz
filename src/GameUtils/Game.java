...
public class Game implements Runnable {
    ...
    private final double UPDATE_CAP = 1.0/TARGET_FPS;
    ...
    public Game(JFrame _frame, int sizeX, int sizeY){
        ...
        frame.addKeyListener(input_listener);
        pane.addMouseMotionListener(input_listener);
        pane.addMouseListener(input_listener);
        keys=new boolean[256];
        mouse=new Mouse();
    }
    public void add(Object obj) throws IllegalArgumentException{
        if(obj instanceof RenderObj) renders.add((RenderObj)obj);
        else if(obj instanceof Updater) updaters.add((Updater)obj);
        else throw new IllegalArgumentException("Cannot add non-RenderObj or non-Updater to Game");
    }
    ...
    public void run(){ //Main Game Loop
        ...
        while(running){
            ...
            while(unprocessedTime>=UPDATE_CAP){
                ...
                //Update inputs:
                keys=input_listener.getKeys();
                mouse=input_listener.getMouse();
                //Update objects:
                for(int i=0; i<updaters.size(); i++){
